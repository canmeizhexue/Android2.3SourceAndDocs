...
public class ActivityStack {
    ...
    /**请求关闭token代表的页面，如果页面将要关闭，那么返回true.
    ...
    final boolean finishActivityLocked(ActivityRecord r, int index,
        ...
        if (index < (mHistory.size()-1)) {
            ...
            if (next.task == r.task) {
                if (r.frontOfTask) {
                		//如果当前页面是task的顶，并且这个task中还有其他Activity,那么标记其他页面为task的新的顶
    ...
    final void cleanUpActivityLocked(ActivityRecord r, boolean cleanServices) {
		...
		//1.将ActivityRecord从mHistory中删除，
		//2.将ActivityRecord标记不在列表里面，标记已经DESTROYED
		//3.删除WMS中的AppWindowToken
		//4.清除这个Activity的一些资源，比如清除服务链接
		//5.清除这个Activity的uri权限
    private final void removeActivityFromHistoryLocked(ActivityRecord r) {
        if (r.state != ActivityState.DESTROYED) {
        		//这个地方也表明，DESTROYED状态的Activity不会在mHistory列表里面
            mHistory.remove(r);
            r.inHistory = false;
            r.state = ActivityState.DESTROYED;
            //将对应的AppWindowToken从相应的列表删除，
