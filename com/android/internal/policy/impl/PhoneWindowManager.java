 ...
 *在SystemServer的主线程里面创建WMS时候附带创建的，整个系统只有一个，在PolicyThread线程里面调用init初始化
 */
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    /** {@inheritDoc} 这个函数不是在创建PhoneWindowManager对象的线程里面执行的，而是在PolicyThread线程执行的，PhoneWindowManager对象是在SystemServer中的线程里面创建的*/
    ...
    /** {@inheritDoc} */
    //根据窗口类型，判断调用者是否有权限添加想要类型的窗口，非系统级别的窗口，都是可以直接通过的。
    ...
    /**
    *根据窗口类型，调整窗口参数，比如Toast窗口是不需要和用户交互的，应该不能获取焦点，也不能获取触摸屏事件，
    *所以客户端对于这种类型，即使设置了能获取焦点和触摸屏事件也不会起作用的
    */
    ...
    /** 给出窗口的可用大小，不是窗口的最终大小{@inheritDoc} */
