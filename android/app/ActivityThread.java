...
public final class ActivityThread {
    ...
    //ActivityThread的一个成员变量，在ActivityThread的attach方法里面传递给AMS，主要用来从AMS向应用程序传递消息，被AMS调用
    private final class ApplicationThread extends ApplicationThreadNative {
        ...
        // 说明Intent是可以跨进程传播的，ActivityInfo可以跨进程，是AMS创建，被AMS调用，
        public final void scheduleLaunchActivity(Intent intent, IBinder token, int ident,
                ActivityInfo info, Bundle state, List<ResultInfo> pendingResults,
                List<Intent> pendingNewIntents, boolean notResumed, boolean isForward) {
            ActivityClientRecord r = new ActivityClientRecord();

            r.token = token;//此处token是这个Activity在AMS中的唯一标识
            r.ident = ident;
            r.intent = intent;
            r.activityInfo = info;
            r.state = state;
        ...
        public final void bindApplication(String processName,
            ...
            // 注意，此处并没有给info字段赋值，因为LoadedApk不能跨进程传输，所以info字段不是从AMS传递过来的
            AppBindData data = new AppBindData();
            data.processName = processName;
            data.appInfo = appInfo; //实现了Parcelable
            data.providers = providers;//实现了Parcelable
            data.instrumentationName = instrumentationName;//实现了Parcelable
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case LAUNCH_ACTIVITY: {
                    ...
                    //这个地方获取到的LoadedApk刚好是之前初始化Application时生成的那个，俩个共用一个LoadedApk
    ...
    private final Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
            ...
            if (activity != null) {
                ...
                // Activity对象会在ActivityClientRecord中存一份
                r.activity = activity;
                r.stopped = true;
    ...
    private final void handleCreateService(CreateServiceData data) {
        ...
        // we are back active so skip it.
        unscheduleGcIdler();
        //获取到的依然是初始化Application时创建的LoadedApk
        LoadedApk packageInfo = getPackageInfoNoCheck(
                data.info.applicationInfo);
        ...
        try {
            ...
            service.attach(context, this, data.info.name, data.token, app,
                    ActivityManagerNative.getDefault());
            //attach之后才是调用onCreate函数
            service.onCreate();
            mServices.put(data.token, service);
            try {
                // 通知AMSService对象已经创建好了
        ...
        if (r != null && !r.activity.mFinished) {
            ...
            try {
                ...
                if (r.pendingResults != null) {
                    //从这个地方可以看到会先将结果发给Activity，然后调用它的onResume
    ...
    final void handleResumeActivity(IBinder token, boolean clearHide, boolean isForward) {
        ...
        if (r != null) {
            ...
            if (r.window == null && !a.mFinished && willBeVisible) {
                ...
                a.mDecor = decor;
                // performResumeActivity的时候才将View添加到WindowManager，并且在这个地方更改了窗口类型
    ...
    //调用Activity的dispatchActivityResult
    ...
    //在handler中调用，，，这类函数以handler开头，这段代码执行的时间会非常早，会在这里创建Application对象
    private final void handleBindApplication(AppBindData data) {
        ...
        if (data.instrumentationName != null) {
            //Android Unit Test的时候这个条件才会满足，一般情况下这个条件是不会满足的
            ContextImpl appContext = new ContextImpl();
        ...
        // info字段的类型是LoadedApk，第二个参数为null，那么久不会执行Application的onCreate方法
        Application app = data.info.makeApplication(data.restrictedBackupMode, null);
        mInitialApplication = app;

        List<ProviderInfo> providers = data.providers;
        if (providers != null) {
            //从这个地方可以看到初始化Provider在Application的onCreate函数之前就执行了,传入的Context是Application对象
    ...
    // ProviderInfo可以跨进程传递，在初始化Application的过程中调用
    ...
    //应用程序的入口函数  ActivityThread.main----->ActivityThread.attach
    public static final void main(String[] args) {
        ...
        //注意是准备了Looper之后才new的ActivityThread,而这个对象初始化的时候会new一个Handler
        ActivityThread thread = new ActivityThread();
        thread.attach(false);
