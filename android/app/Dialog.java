...
/**Dialog对象的context一般也是Activity，dialog中的WindowManager就是要从这个context中获取的。
 * Dialog对象对应的窗口默认类型是TYPE_APPLICATION，和Activity是一样的。也就是说默认情况下Dialog并不是Activity的子窗口
...
public class Dialog implements DialogInterface, Window.Callback,
    ...
    public Dialog(Context context, int theme) {
        ...
        mWindowManager = (WindowManager)context.getSystemService(Context.WINDOW_SERVICE);//如果之前传入的context是Activity的话，那么这个windowManager就是创建Activity时候的LocalWindowManager
        Window w = PolicyManager.makeNewWindow(mContext);//也会创建一个PhoneWindow对象
        mWindow = w;
        w.setCallback(this);
        w.setWindowManager(mWindowManager, null, null);//这个里面会继续创建一个LocalWindowManager，传入的参数是被代理的对象
        //但是这个地方要注意，并没有在给Window对象设置windowManager后再重新获取WindowManager对象，
