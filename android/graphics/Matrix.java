...
/**matrix3*3的矩阵，初始状态是单元矩阵identity matrix
 * R代表运算后的矩阵，M代表当前矩阵，T代表转换矩阵
 * 总共可以分为translate（平移），rotate（旋转）、scale（缩放）、skew（倾斜）四种变换，
 * 每一种变换都提供了三种set、post、pre三种操作方式，
 * set相当于直接在一个新的单元矩阵上进行操作，然后把结果值赋值给当前矩阵，不用管当前矩阵的当前值
 * pre操作是和名字相反的，是当前矩阵右乘转换矩阵,R=M  *  T
 * post操作也是和名字有点相反，是当前矩阵左乘转换矩阵，R=T  *  M
...
public class Matrix {
    ...
    */
    ...
    /** 恢复成单元矩阵Set the matrix to identity */
    public void reset() {
        native_reset(native_instance);
    }

    /**相当于直接在一个新的单元矩阵上进行操作，然后把结果值赋值给当前矩阵，不用管当前矩阵的当前值 Set the matrix to translate by (dx, dy). */
    public void setTranslate(float dx, float dy) {
        native_setTranslate(native_instance, dx, dy);
    }

    /**相当于直接在一个新的单元矩阵上进行操作，然后把结果值赋值给当前矩阵，不用管当前矩阵的当前值
    ...
    /**相当于直接在一个新的单元矩阵上进行操作，然后把结果值赋值给当前矩阵，不用管当前矩阵的当前值 Set the matrix to scale by sx and sy. */
    ...
    /**相当于直接在一个新的单元矩阵上进行操作，然后把结果值赋值给当前矩阵，不用管当前矩阵的当前值 Set the matrix to rotate by the specified sine and cosine values. */
    ...
    /**相当于直接在一个新的单元矩阵上进行操作，然后把结果值赋值给当前矩阵，不用管当前矩阵的当前值 Set the matrix to skew by sx and sy. */
