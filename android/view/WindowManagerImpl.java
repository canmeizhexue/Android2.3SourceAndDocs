...
public class WindowManagerImpl implements WindowManager {
            ...
            if (wparams.type >= WindowManager.LayoutParams.FIRST_SUB_WINDOW &&
                ...
                for (int i=0; i<count; i++) {
                    if (mRoots[i].mWindow.asBinder() == wparams.token) {
                        // 如果创建的是子窗口，那么它要和他父窗口token指向同一个W对象
    ...
    //先找到这个view的ViewRoot，然后找到同一个位置的WindowManager.LayoutParams
    ...
    //从src中删除下标为index的元素，同时把其他元素复制到dst数组
    ...
    //查找指定的view是否在数组中，存在返回下标。不存在的处理依赖于第二个参数，当required为true的时候，抛出异常，否则返回-1
