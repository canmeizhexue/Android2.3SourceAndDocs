...
/**1.InputHandler.handleMotion-->ViewRoot.dispatchMotion--->dispatchPointer---->发送异步消息，异步调用deliverPointerEvent---》mView.dispatchTouchEvent
 * 2.InputHandler.handleKey-->dispatchKey-->deliverKeyEvent--->优先mView.dispatchKeyEventPreIme，没被处理时--》deliverKeyEventToViewHierarchy---》mView.dispatchKeyEvent
 *
...
public final class ViewRoot extends Handler implements ViewParent,
    ...
    final W mWindow;  //不是静态的变量，说明和ViewRoot是一一对应关系
    ...
    boolean mLayoutRequested;//标记是否请求了layout
    boolean mFirst;
    ...
    boolean mAdded;//标记View树是否被添加到WindowManager中
    ...
    // 静态方法，获取WindowSession变量
    public static IWindowSession getWindowSession(Looper mainLooper) {
        synchronized (mStaticInit) {//这个锁是一个静态变量，只不过不符合规范而已
            if (!mInitialized) {//这也是一个静态变量，只不过命名不太符合规范
                try {
    ...
    // 构造函数，会创建AttachInfo对象
    public ViewRoot(Context context) {
        // context是DecorView里面的Context对象，而DecorView中的Context对象是从PhoneWindow对象里面的，
        //对于Activity而言，PhoneWindow中的context就是Activity对象
        super();
    ...
    */
    ...
    /**被WindowManagerImpl对象里面调用--会触发一次requestLayout
     * We have one child
     */
        ...
        synchronized (this) {
            if (mView == null) {
                ...
                if (panelParentView != null) {
                    //如果存在父窗口，照这种逻辑，假如A是某个Activity的子窗口，B是A的子窗口，那么B和A的mPanelParentWindowToken都是这个Activity的W对象
                ...
                try {
                    // mWindow对象是W类的，，完成真正的窗口添加操作
                    //每个应用程序只有一个sWindowSession，对应WMS中Session的子类，WMS为每一个应用程序分配一个Session
                ...
                if (view instanceof RootViewSurfaceTaker) {
                    // DecorView就是这个，那么对应Activity的时候也是走的这条路
                    mInputQueueCallback =
    ...
    private void performTraversals() {
        ...
        if (DBG) {
            ...
            host.debug();
				...
				//host代表的是view树，mAdded代表是否添加到窗口管理器上面
        if (host == null || !mAdded)
            return;
        ...
        if (mFirst) {
            //第一次遍历会调用dispatchAttachedToWindow
        ...
        if (mLayoutRequested) {
            //如果之前请求过layout-----这里会调用View树的measure函数
        ...
        final boolean didLayout = mLayoutRequested; // 之前请求过layout
    ...
    private void draw(boolean fullRedrawNeeded) {
    	//1.检查Surface有效性，一般都是有效的
    	//mSurface是构造对象的时候就初始化了
        Surface surface = mSurface;
        if (surface == null || !surface.isValid()) {
            return;
        }
				//第一次绘制之后的钩子回调，这个地方对于想完成在第一次绘制结束后的某个时间点执行某些操作而言是很有用的
				//这个变量只在这里进行赋值
        if (!sFirstDrawComplete) {
            synchronized (sFirstDrawHandlers) {
        ...
        //这个函数绝大多数情况下，内部的执行流程可以忽略，主要是考虑完备性问题，我们只抓主流程
        scrollToRectOrFocus(null, false);
				//如果滚动状态发生变化，那么将这一个事件发送出去
        ...
        Rect dirty = mDirty;
        if (mSurfaceHolder != null) {
        	//如果有SurfaceHolder,那么有应用程序自己去绘制，这条路径暂时还没有找到什么条件下会满足
            ...
            if (!dirty.isEmpty()) {
                ...
                if (mGL != null && canvas != null) {
                    ...
                    try {
                        ...
                        //DisplayMetrics.DENSITY_DEVICE的值，在系统启动时被设置，不同机器设置的值可能为240，160，120
                        canvas.setScreenDensity(scalingRequired
                                ? DisplayMetrics.DENSITY_DEVICE : 0);
                        //这个地方才是真正的视图树的绘制工作，
                        //为什么这个绘制操作看起来和之前的脏区域没有关系呢，当然可能使用GL的时候效率比较高，整个屏幕都重新绘制了，而不使用GL方式时，可能为了效率只绘制脏区域
                        mView.draw(canvas);
                        if (Config.DEBUG && ViewDebug.consistencyCheckEnabled) {
        ...
        if (!dirty.isEmpty() || mIsAnimating) {
            ...
            try {
                ...
                //说明这个地方可能会去修改dirty，同时这个canvas还和这个脏区域有关
    ...
    // 被doDie方法调用，会调用view树的dispatchDetachedFromWindow
    ...
    /**
     * 这次输入事件处理完了，通过回调告诉输入队列，输入事件已经处理完毕
     */
   ...
   // InputMethodCallback.finishedEvent
    ...
    static class W extends IWindow.Stub {
        ...
        public W(ViewRoot viewRoot, Context context) {
            mViewRoot = new WeakReference<ViewRoot>(viewRoot);
				...
				//WMS通知这个窗口，它的大小改变了，需要调整，比如有弹出输入法窗口，这个窗口大小改变了。或者关闭了输入法窗口，这个窗口大小改变了
    ...
    /**RunQueue用ArrayList主要是存储了一些异步操作
     * @hide
     */
