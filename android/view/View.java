...
public class View implements Drawable.Callback, KeyEvent.Callback, AccessibilityEventSource {
    ...
    */
    ...
    /**当前view到父view的距离，因为父容器滚动内容时，子view的layout位置信息并没有改变
     * 这个也说明不是每时每刻mLeft都是真正的在父容器中的位置，需要和滚动坐标一起才能确定真正的位置
    ...
    protected int mLeft;
    /**当前view的右边到父view的左边的距离，这个值完全有可能超过父view的边界，比如在xml文件中写死这个view的宽度很大
     * The distance in pixels from the left edge of this view's parent
    ...
    @ViewDebug.ExportedProperty(category = "layout")
    protected int mTop;
    /**当前view的底部到父view顶部的距离，这个值完全有可能超出父view的边界，比如xml中设置这个view的高度很大，如30000dp
     * The distance in pixels from the top edge of this view's parent
    ...
    /**相当于将监听器进行暂时保存，一旦这个view树添加到窗口管理器，会将这些监听器合并到那个共用的ViewTreeObserver上
    ...
    */
    ...
    /**回调长按监听函数，如果没有消费，那么显示上下文菜单。这里表明长按的回调优先级高于上下文的显示，而且俩个机制是互斥的
    ...
    /**调用onTouchListener和ouTouchEvent函数
    ...
    /**在整个屏幕上，当前view所在的窗口能够占据的大小，返回的矩形是以全局坐标系为参考的
     * 对于全屏和非全屏的Activity而言，获取的结果是不一样的，非全屏的话需要减去状态栏的高度，也就是说非全屏的话，outRect.top刚好是状态栏的高度值
     * 在Dialog和PopupWindow中获取的结果和Activity中获取的结果一样,表示这些view能够在哪些区域进行绘制
    ...
    /**里面实现各种监听事件的回调，三种状态tap(prepress)--press---longpress
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (((viewFlags & CLICKABLE) == CLICKABLE ||
                (viewFlags & LONG_CLICKABLE) == LONG_CLICKABLE)) {
            //判断是否可以点击，如果不能点击，那么不消费事件，直接返回false
            //如果可以点击，那么消费事件，直接返回true
            //这里不会再有转发函数
            //这里有俩个时间点ViewConfiguration.getTapTimeout()和ViewConfiguration.getLongPressTimeout()

            //俩个时间点分成三段，在不同的时间段内收到up消息，就分别认为是tap、pressed、longclick事件
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if ((x < 0 - slop) || (x >= getWidth() + slop) ||
                        ...
                        //出现在边界外了，因为down事件才会判断边界，所以这个地方有可能出边界
                        // Outside button
                        removeTapCallback();
    ...
    /**返回当前应该绘制区域的边界，要把滚动因素考虑进去，是以自己为左上角为坐标原点
     * 或者说以当前view的左上角为原点，内容的哪一个部分应该被绘制出来（最终有可能被裁剪了）
    ...
    /**测量得到的宽度，一般在layout阶段作为参考，不一定被使用
    ...
    /**测量得到的高度，一般在layout阶段作为参考，不一定被使用
    ...
    /**view在父容器内容坐标系中的位置坐标，完全有可能超出屏幕大小，超出的部分只是不显示而已
    ...
    /**view在父容器内容坐标系中的位置坐标,完全有可能超出屏幕大小
    ...
    /**
     * 获取全局坐标，
     * 1.对于Activity而言，获取到的的确是android绝对坐标系的坐标，不管Activity是否全屏
     * 2.对于dialog或者popupwindow而言获取到的是相对于他们的root的坐标系
     * @param r
     * @return
     */
    ...
    /**
     * 获取view的可见区域，坐标相对于自身，因为view的区域有可能部分不可见，绘制的时候被裁剪了，
     * 所以才会有获取可见坐标
     * @param r 输出参数
     * @return
     */
    ...
    /**滚动view的内容，view的坐标位置是不会变的，变的是里面的内容，
    ...
    /**从AttachInfo对象里面获取，返回这个窗口的token信息，如果这个窗口是一个子窗口，那么返回值是null，否则是这个窗口的W对象
    ...
    /**从AttachInfo对象里面获取，，返回父窗口的token(W对象)，否则返回自己的token(W对象)
    ...
    // 这个view树从Window上摘下来了，将本地的mAttachInfo置为null，dispatch开头的函数一般都是被外部调用的
    ...
    /**保存状态数据，必须保存父view的状态数据，这个地方的标记位表明所有父类的方法都必须调用，并且最顶层的是BaseSavedState.EMPTY_STATE
     * 父类(注意不是父view)的状态保存在子类的Parcelable中
    ...
    /**恢复数据，用状态位来保证父类的方法必须被调用，同一个view树种如果俩个不同类型的view用了同一个id，那么是会抛异常的，因为继承层次不一样
     * 但是同类型的用了同一个id，不会抛异常。但是会造成数据丢失
     * 为了避免出错，同一个view树的最好不要用相同id
    ...
    public void draw(Canvas canvas) {
        ...
        if (!dirtyOpaque) {
        	//脏区域透明，需要绘制背景
        	//cancas的getWidth和getHeight是父视图的宽和高，
        	//同一个ViewGroup的子view的canvas的宽高都是一样的，只是裁剪区getClipBound不一样而已
    ...
    /**给子树和自己设置尺寸和位置信息，坐标是相对于父容器的内容坐标系，这是一个模板方法，子类无法复写
    ...
    protected boolean setFrame(int left, int top, int right, int bottom) {
        ...
        if (mLeft != left || mRight != right || mTop != top || mBottom != bottom) {
            ...
            //先去绘制，把原来占有的地方进行重新绘制，然后再把位置信息赋值给成员变量
    ...
    /**如果这个view树还没有添加到窗口管理器，那么会返回一个临时的ViewTreeObserver，我们依然可以往这个对象里面设置监听
     *一旦这个view树添加到窗口管理器，相应的监听器会被合并到整个view树共用的那个ViewTreeObserver
    ...
    /**计算这个view相对于屏幕绝对坐标系的位置，
     * 1.特别注意的是，真对于对话框，它看起来是在中央，其实在Activity非全屏的情况下，它并不是在绝对坐标系的正中央，而是在Activity窗口坐标系的正中央
    ...
    /**计算view的左上角的点在window中的坐标
     * Activity的话，就在Activity所在window中的坐标
     * Dialog的话，就在Dialog所在window中的坐标，可能值很小，尽管对话框在中间甚至底部
     * PopupWindow的话，就在popupWindow中的坐标，可能值很小，尽管对话框在中间甚至底部
    ...
    public void getLocationInWindow(int[] location) {
        ...
        while (viewParent instanceof View) {
            // 因为父容器滚动内容时，子view的layout位置信息并没有改变,需要减去这个滚动偏移，才能得到真正的位置信息
        ...
        if (viewParent instanceof ViewRoot) {
            ...
            // 因为父容器滚动内容时，子view的layout位置信息并没有改变,需要减去这个滚动偏移
    ...
    static class AttachInfo {
        ...
        final IWindow mWindow;//每个窗口都有的，

        final IBinder mWindowToken; //代表这个窗口的ViewRoot的W对象，是mWindow通过asBinder得到的，并不是书上说的和mPanelParentWindowToken互斥
        ...
        IBinder mPanelParentWindowToken; //如果当前窗口是子窗口，那么这个变量就是顶级窗口（最上面的Activity对应的窗口）的W对象，
        Surface mSurface;
