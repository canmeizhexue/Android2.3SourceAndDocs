...
public abstract class Window {
    ...
    */
    ...
    private IBinder mAppToken;// 如果这个Window对应的是Activity，那么这个mAppToken对应的是ActivityRecord，与Activity里面的token相同，否则的话是null
    ...
    // The current window attributes.//每个Window对象都有一个这个对象，，默认类型TYPE_APPLICATION
    ...
    public void setWindowManager(WindowManager wm,
        ...
        mAppToken = appToken;//当这个Window对应的是Activity时，mAppToken是ActivityRecord，当创建的是Dialog对话框时，这个地方传入的是null
        mAppName = appName;
        if (wm == null) {
            // Activity对应的Window类这个地方传入的是null
            wm = WindowManagerImpl.getDefault();// 同一个App中获取到的是同一个对象
        }
    ...
    private class LocalWindowManager implements WindowManager {
        ...
        public final void addView(View view, ViewGroup.LayoutParams params) {
            ...
            if (wp.type >= WindowManager.LayoutParams.FIRST_SUB_WINDOW &&
                ...
                //子窗口，，如果它的token为null，，那么将子窗口的token设置为父窗口的W对象
                if (wp.token == null) {
                    View decor = peekDecorView();
                    if (decor != null) {
                        wp.token = decor.getWindowToken(); //W对象
                    }
                }
                ...
                //不是子窗口，，layoutParams中的token值是代表Activity的token值
                if (wp.token == null) {
    ...
    /**给WindowManager.LayoutParams.flags设置不同的值
